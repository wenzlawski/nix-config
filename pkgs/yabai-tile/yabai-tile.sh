...
display_height=$(echo "$display_frame" | jq '.h' | awk '{print int($1) - 25}')
half_display_width=$(echo "$display_width / 2" | bc)
third_display_width=$(echo "$display_width / 3" | bc)
half_display_height=$(echo "$display_height / 2" | bc)
...
if [ "$DIRECTION" == left ]; then
	echo "Tiling left"
	CHECK_1_0=$((window_width == half_display_width - 1 && window_height == display_height))
	...
	echo "tiling up"
	CHECK_1_0=$((window_height == half_display_height && window_width == display_width))
	CHECK_1_1=$((window_y == 0))
	TILE_1="2:2:0:0:1:1" # top left 1/4
	TILE_0="2:1:0:0:1:1" # top 1/2

	CHECK_2_0=$((window_height == half_display_height && window_width == half_display_width - 1))
	...
	echo "tiling down"
	CHECK_1_0=$((window_height == half_display_height && window_width == display_width))
	CHECK_1_1=$((window_y == half_display_height + 2))
